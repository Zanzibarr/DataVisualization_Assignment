    ...
    def load_data(name : str) -> pl.DataFrame:
        return (
            ...
            .with_columns(
                ...
                Time = pl.when(pl.col("Status").is_in([0,1])).then(pl.col("Time") - pl.col("Parsing_Time")).otherwise(pl.lit(900)),
            )
...
def _(load_data, pl):
    ...
    tl_hLM = load_data("run_tl_e+lmcut_h__251112.csv").with_columns(Model = pl.lit("TL_hLM"))
    # VERTEX ELIMINATION
    ...
    ve_hLM = load_data("run_ve_e+lmcut_h__251113.csv").with_columns(Model = pl.lit("VE_hLM"))
    ...
    sec_hLM = load_data("run_sec_e+lmcut_h__251115.csv").with_columns(Model = pl.lit("SEC_hLM"))
    ...
    lmc_hLM = load_data("run_comp_e+lmcut_h__251112.csv").with_columns(Model = pl.lit("LMC_hLM"))
    flm_hLM = load_data("run_base_min_1k_noworse_sortvf_lh10__260204.csv").with_columns(Model = pl.lit("LMCf_hLM"))
...
def _(pl):
    ...
    def compare_data(
        ...
        # Define group order
        group_order = ["all", "solvable", "all-solvable", "[0,1)", "[1,10)", "[10,100)", "[100,900)", "[900,+inf)"]
        group_order_map = {g: i for i, g in enumerate(group_order)}

        data = (
            data.with_columns(
                pl.col("Model").replace(order_map).alias("model_order"), # Associate to each line a number for the model order
                pl.col("Group").replace(group_order_map).alias("group_order") # Associate to each line a number for the group order
            )
            .sort(by=["group_order", "model_order"]) # Sort first based on group order, then on model order
        ...
        data = data.with_columns(
            ...
            Model_Class=pl.col("Model").cast(pl.String).str.slice(0, 2)
        )
...
def _(categorize, compare_data, data_folder, gg, pl):
    ...
    def bar_plot(
        ...
        plot = (
            gg.ggplot(
                data,
                gg.aes(x="Group", y="sgm_ratio", fill="Model", group="Model")
            ) +
    ...
    def bar_plot_multi_baseline(
        ...
        ordered_classes = class_order if class_order is not None else list(class_baselines.keys())
        ...
        return (
            ...
            gg.scale_linetype_manual(
                name="Reference",
                values={v: "dashed" for v in class_baselines.values()}
            ) +
            gg.labs(
                title="Model Performance Comparison",
    ...
    def time_nodes_plot(
        ...
        if class_baselines is None:
            ...
            compared = pl.concat([
                compare_data(categorized, m, baseline, comparisons)
            ...
            for cls, cls_base in class_baselines.items():
                ...
                cls_models = [cls_base] + [m for m in comparisons if m.startswith(cls_base)]
                cls_comparisons = cls_models[1:]

                # Keep only instances solved by every model in this class
                cls_data = categorize(
                    data.filter(pl.col("Model").cast(pl.String).is_in(cls_models))
                        .filter(pl.col("Model").count().over("Instance") == len(cls_models))
                )

                for metric in metric_order:
                    frames.append(
                        compare_data(cls_data, metric, cls_base, cls_comparisons)
...
def _(flm_hLM, lmc_hLM, run_best, time_nodes_plot, ve, ve_hLM):
    time_nodes_plot([ve, ve_hLM, lmc_hLM, flm_hLM, run_best], "VE", ["VE_hLM", "LMC_hLM", "LMCf_hLM", "LMC_b"])
    return
...
def _(
    ...
    def solved_bar_plot(data: pl.DataFrame) -> gg.ggplot:
        plot_data = (
            ...
            .sort("Solved", descending=False)  # ascending so that highest is on top in coord_flip
            .with_columns(
                pl.col("Model").cast(pl.Enum(
                    data.filter(pl.col("Status") == True)
                    .group_by("Model").agg(pl.len().alias("Solved"))
                    .sort("Solved", descending=False)
                    .get_column("Model").to_list()
                ))
            ).with_columns(
                model_type=pl.col("Model").cast(pl.String).str.slice(0, 2)
            )
        )
        ...
        return (
            ...
            gg.annotate("segment", x=0.4, xend=10.6, y=500, yend=500, color="white", size=0.8) +
            gg.annotate("segment", x=0.4, xend=10.6, y=1000, yend=1000, color="white", size=0.8) +
            gg.annotate("segment", x=0.4, xend=10.6, y=1500, yend=1500, color="white", size=0.8) +
            gg.annotate("segment", x=0.4, xend=10.6, y=2000, yend=2000, color="white", size=0.8) +
            gg.annotate("segment", x=0.4, xend=10.6, y=2500, yend=2500, color="white", size=0.8) +
